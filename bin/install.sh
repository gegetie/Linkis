...
if test -n "$EUREKA_SERVICE_URL"
then
  EUREKA_URL=$EUREKA_SERVICE_URL
fi
...
SERVER_NAME=linkis-ujes-impala-enginemanager
...
ENGINE_CONF_PATH=$SERVER_HOME/$SERVER_NAME/conf/linkis-engine.properties
executeCMD $SERVER_IP   "sed -i ${txt}  \"s#wds.linkis.enginemanager.sudo.script.*#wds.linkis.enginemanager.sudo.script=$SERVER_HOME/$SERVER_NAME/bin/rootScript.sh#g\" $SERVER_CONF_PATH"
executeCMD $SERVER_IP   "sed -i ${txt}  \"s#wds.linkis.enginemanager.core.jar.*#wds.linkis.enginemanager.core.jar=$SERVER_HOME/$SERVER_NAME/lib/linkis-ujes-impala-engine-$LINKIS_VERSION.jar#g\" $SERVER_CONF_PATH"
SERVER_ENGINE_CONF_PATH=$SERVER_HOME/$SERVER_NAME/conf/linkis-engine.properties
executeCMD $SERVER_IP   "sed -i ${txt}  \"s#\#hadoop.config.dir.*#hadoop.config.dir=$HADOOP_CONF_DIR#g\" $SERVER_ENGINE_CONF_PATH"
executeCMD $SERVER_IP   "sed -i ${txt}  \"s#wds.linkis.engine.impala.coordinator.hosts.*#wds.linkis.engine.impala.coordinator.hosts=$IMPALA_COORDINATOR_HOST#g\" $SERVER_ENGINE_CONF_PATH"
isSuccess "subsitution linkis.properties of $SERVER_NAME"
echo "<----------------$SERVER_NAME:end------------------->"
...
PACKAGE_DIR=linkis/ujes/impala
SERVER_NAME=linkis-ujes-impala-entrance
