...
/**
 * created by enjoyyin on 2018/10/16 Description: an implementation class of
...
public class EntranceRestfulApi implements EntranceRestfulRemote {
	...
	/**
	 * The execute function handles the request submitted by the user to execute the
	 * task, and the execution ID is returned to the user.
	 * execute函数处理的是用户提交执行任务的请求，返回给用户的是执行ID
	 * 
	 * @param json
	 *            Incoming key-value pair(传入的键值对)
	 * @return Repsonse
	 */
	@Override
	@POST
	...
	public Response execute(@Context HttpServletRequest req, Map<String, Object> json) {
		...
		json.put(TaskConstant.LOGINUSER, SecurityFilter.getLoginUsername(req));
		...
		pushLog(LogUtils.generateInfo("You have submitted a new job, script code (after variable substitution) is"),
				job);
		pushLog("************************************SCRIPT CODE************************************", job);
		pushLog(((RequestPersistTask) task).getCode(), job);
		pushLog("************************************SCRIPT CODE************************************", job);
		pushLog(LogUtils.generateInfo("Your job is accepted,  jobID is " + execID + " and taskID is " + taskID
				+ ". Please wait it to be scheduled"), job);
	...
	public Response log(@Context HttpServletRequest req, @PathParam("id") String id) {
		...
		if (job.isDefined()) {
			...
			message = Message.error("Can't find execID(不能找到execID): " + id
	...
	// this.entranceServer.getEntranceContext().getOrCreatePersistenceManager();
	// Message message = null;
	// try {
	...
	public Response kill(@PathParam("id") String id) {
		...
		if (job.isEmpty()) {
			message = Message
					.error("Can't find execID(不能找到execID): " + id + "Corresponding job, can't kill(对应的job，不能进行kill)");
			...
			try {
				...
				if (job.get() instanceof EntranceJob) {
					...
					this.entranceServer.getEntranceContext().getOrCreatePersistenceManager().createPersistenceEngine()
							.updateIfNeeded(task);
				}
				...
				message = Message
						.error("An exception occurred while killing the job, kill failed(kill job的时候出现了异常，kill失败)");
				message.setMethod("/api/entrance/" + id + "/kill");
				message.setStatus(1);
			}
		}
	...
	public Response backgroundservice(@Context HttpServletRequest req, Map<String, Object> json) {
		...
		json.put(TaskConstant.LOGINUSER, loginUser);
		if (null != json.get(TaskConstant.UMUSER)&&StringUtils.isNotBlank((String)json.get(TaskConstant.UMUSER))) {
			json.put(TaskConstant.UMUSER, (String)json.get(TaskConstant.UMUSER));
		} else {
			json.put(TaskConstant.UMUSER, loginUser);
		}
